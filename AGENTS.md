...
**CI validates pull requests and deploys only after non-PR verification.** `.github/workflows/deploy.yml` runs `check`, `lint`, `test`, and the web export for pull requests targeting `main`, then runs the same verification before a `main` push or manual-dispatch deployment to Pages. Pull-request runs never deploy. Run the same commands locally before pushing so failures are found before CI.
...
- Commit messages: imperative subject, then a body explaining the _why_, including reproduction and verification steps for bug fixes. Follow the style of `9576cf3`.
